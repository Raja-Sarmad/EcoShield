...
import { 
  Plus, Minus, ShoppingBag, ShieldCheck, 
  Truck, RotateCcw, CheckCircle2, Star
} from "lucide-react";
import PreOrderModal from "@/components/home/PreOrderModal";

const bundles = [
  { id: 1, name: "Starter Pack",      tagline: "100ml Spray Bottle",   price: 699,  desc: "Perfect for a single pair of shoes." },
  { id: 2, name: "Essentials Bundle", tagline: "Spray + Cleaning Cloth", price: 899, desc: "Deep clean and shield combo.", popular: true },
  { id: 3, name: "Ultra Saver",       tagline: "2x Spray Pack",        price: 1299, desc: "Best value for your entire collection." },
];

export default function ShopPage() {
  const [qty, setQty]           = useState(1);
  const [selected, setSelected] = useState(bundles[1]);
  const [modalOpen, setModalOpen] = useState(false); // ← modal state

  return (
    <>
      {/* Modal renders outside section — no overflow issues */}
        ...
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
            ...
            >
              ...
              >
                <Image src="/images/bottle3.png" alt="EcoShield Bottle" width={400} height={500} className="mx-auto drop-shadow-2xl rounded-2xl" />
              </motion.div>
            ...
            <div className="lg:col-span-7 space-y-8">
              <div className="bg-white rounded-[2.5rem] p-8 lg:p-10 shadow-sm border border-slate-100">
                <h2 className="text-xl font-bold mb-6 flex items-center gap-2">
                  <ShoppingBag size={20} className="text-emerald-600" /> Select Your Bundle
                </h2>
                  ...
                  {bundles.map(b => (
                    ...
                    >
                      ...
                      <div className="flex justify-between items-center">
                        <div>
                          <p className={`font-bold text-lg ${selected.id === b.id ? "text-emerald-900" : "text-slate-800"}`}>{b.name}</p>
                          <p className="text-xs text-slate-500 font-medium">{b.tagline}</p>
                        </div>
                        <p className={`font-black text-xl ${selected.id === b.id ? "text-emerald-600" : "text-slate-400"}`}>PKR {b.price}</p>
                      </div>
                    </button>
                  ))}
                </div>
                ...
                <div className="flex items-center justify-between p-6 bg-slate-50 rounded-2xl mb-8">
                  ...
                  <div className="flex items-center gap-6">
                    <button onClick={() => setQty(Math.max(1, qty - 1))} className="w-10 h-10 rounded-full bg-white shadow-sm border border-slate-200 flex items-center justify-center hover:bg-emerald-500 hover:text-white hover:border-emerald-500 transition-all">
                      <Minus size={16} />
                    </button>
                    <span className="font-black text-xl w-4 text-center">{qty}</span>
                    <button onClick={() => setQty(qty + 1)} className="w-10 h-10 rounded-full bg-white shadow-sm border border-slate-200 flex items-center justify-center hover:bg-emerald-500 hover:text-white hover:border-emerald-500 transition-all">
                      <Plus size={16} />
                    </button>
                  </div>
                </div>
