  ...
  return (
    <>
      ...
      <nav className={`sticky top-0 z-[100] transition-all duration-300 ${
        scrolled ? "bg-white/80 backdrop-blur-lg shadow-md py-2" : "bg-white py-4"
      }`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-1">
            ...
            <button
              ...
              className="group relative flex items-center gap-2 bg-emerald-600 text-white px-6 py-2.5 rounded-full text-sm font-bold shadow-lg shadow-emerald-200 hover:bg-emerald-700 transition-all active:scale-95 overflow-hidden"
            >
          ...
          <div className="flex lg:hidden items-center gap-3">
            <button
              onClick={() => setModalOpen(true)}
              className="p-2 text-emerald-600 bg-emerald-50 rounded-full active:scale-90 transition-transform"
            >
      ...
      {/* Mobile Menu — OUTSIDE nav so z-index works properly */}
      <AnimatePresence>
        {open && (
          <>
            {/* Backdrop */}
            ...
            <motion.div
              ...
              className="fixed top-0 right-0 h-full w-[80%] max-w-sm bg-white shadow-2xl p-6 flex flex-col"
              style={{ zIndex: 999 }}
            >
              {/* Drawer Header */}
              <div className="flex justify-between items-center mb-8">
                <div className="font-bold text-emerald-600 flex items-center gap-2 uppercase tracking-widest text-xs">
                  <Globe className="w-4 h-4" /> EcoShield
                </div>
                <button
                  onClick={() => setOpen(false)}
              ...
              <div className="pt-6 border-t border-gray-100 mt-4">
                <button
                  ...
                  className="flex items-center justify-center gap-2 w-full bg-emerald-600 text-white py-4 rounded-2xl font-bold text-lg shadow-xl shadow-emerald-200 active:scale-95 transition-all"
                >
